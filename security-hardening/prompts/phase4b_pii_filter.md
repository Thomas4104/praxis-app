Du bist ein Datenschutz-Ingenieur. Dein Auftrag: PII-Daten minimieren die an die Anthropic API gesendet werden, in /Users/thomasbalke/praxis-app/ai/.

WICHTIG: Lies IMMER zuerst die betroffenen Dateien KOMPLETT.

## Aufgabe 1: PII-Filter erstellen
Erstelle: /Users/thomasbalke/praxis-app/ai/pii_filter.py
...
## Aufgabe 2: PII-Filter in Context-Manager einbauen
Datei: /Users/thomasbalke/praxis-app/ai/context.py

Finde die build_context() Methode und wende den Filter an:
```python
from ai.pii_filter import sanitize_context

# Am Ende von build_context(), vor dem Return:
context_text = sanitize_context(context_text)
return context_text
```

## Aufgabe 3: PII-Filter auf Tool-Ergebnisse anwenden
Datei: /Users/thomasbalke/praxis-app/ai/base_agent.py

Finde die Stelle wo Tool-Ergebnisse in die Messages aufgenommen werden:
```python
from ai.pii_filter import sanitize_tool_result

# Nach dem Tool-Aufruf, vor dem Einfuegen in messages:
result = self.tool_executor(tool_block.name, tool_block.input)
result = sanitize_tool_result(tool_block.name, result)  # NEU
```

## Aufgabe 4: AHV-Nummer aus patient_details Tool entfernen
Datei: /Users/thomasbalke/praxis-app/blueprints/patients/ai_tools.py

Finde das Tool das Patienten-Details zurueckgibt und entferne sensible Felder aus der Rueckgabe:
- Entferne: ahv_nummer/ahv_number
- Entferne: insurance_number/versicherungsnummer
- Entferne: address/adresse Details
- Behalte: Name, Geburtsjahr (nicht volles Datum), Versicherungstyp, Patienten-Nummer

## Aufgabe 5: Rate-Limiting fuer KI-Chat
Datei: /Users/thomasbalke/praxis-app/blueprints/dashboard/routes.py

Finde die Chat-API-Route und fuege spezifisches Rate-Limiting hinzu:
```python
@limiter.limit("20 per hour")  # Max 20 KI-Anfragen pro Stunde
```

## Reihenfolge:
1. Lies ai/context.py, ai/base_agent.py, und alle ai_tools.py Dateien
2. Erstelle ai/pii_filter.py
3. Integriere in context.py
4. Integriere in base_agent.py
5. Bereinige patient_details Tool
6. Fuege Rate-Limiting hinzu
7. Syntax-Checks
